...
function CalmDownandGamble:OnInitialize()
	...
	self.chat = {
		...
		CHANNEL_CONSTS = { 
    		...
    		{ label ="Custom" ,   const = "CHANNEL", addon_const = "CHANNEL", callback = "CHAT_MSG_CHANNEL" },                     -- Index 5
		}
	}	
...
function CalmDownandGamble:StartGame()
	...
	if ((self.chat.channel.const == "CHANNEL") and (self.db.global.custom_channel.index == nil)) then 
		...
		--SendChatMessage("Just about to start a round in a custom channel! To join in use /cdg joinChat or /join ".. self.ui.gold_amount_entry:GetText(), "GUILD")
		SendChatMessage("Just about to start a round in a custom channel! To join in use /cdg joinChat or /join "..self.ui.gold_amount_entry:GetText(), "PARTY")
		self:ResetGameStage()
	end
	...
	if (self.chat.channel.const == "CHANNEL") then 
		self:MessageChat("Join the channel by using /join "..self.db.global.custom_channel.name.. " or /cdg join") 
	end
